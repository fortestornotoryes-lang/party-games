            ...
            <div className="text-center space-y-7 px-10 relative z-10">
                ...
                <div className="space-y-3">
                    ...
                    <div>
                        <Typography.Label
                            size="sm"
                            className={`uppercase font-black tracking-[0.45em] mb-2 transition-colors duration-300 ${accent.text}`}
                        >
                            {playerLabel}
                        </Typography.Label>
                        ...
                        <div
                            className="relative inline-flex delay-1000 items-center justify-center px-6 py-3 bg-black/40 border border-white/5 rounded-premium-xs overflow-visible">
                ...
                SYS_ERR
                            ...
                            <span
                                className="absolute right-1 top-1 text-pico font-mono text-premium-pink/40 select-none">
                [v4.02]
              </span>

                            {/* Задняя плашка помех, которая взрывается глитчами */}
                            <div
                                className="absolute inset-0 bg-white/5 opacity-5 animate-matrix-glitch pointer-events-none z-0"/>

                            <Typography.Heading
                                className={`font-display font-black italic tracking-tighter  leading-none relative z-10 select-none text-4xl uppercase`}
                            >
                                {playerName}

                                {/* Glitch Layer 1 - Cyan / Мощное смещение */}
                                <span
                                    className="absolute top-0 left-0 w-full h-full text-premium-cyan opacity-100 mix-blend-screen animate-cyber-hard-1 select-none pointer-events-none z-[-1] before:content-['_'] after:content-['_']">
                  {playerName}
                </span>

                                {/* Glitch Layer 2 - Pink-Red / Противоположный разрыв */}
                                <span
                                    className="absolute top-0 left-0 w-full h-full text-premium-pink opacity-100 mix-blend-screen animate-cyber-hard-2 select-none pointer-events-none z-[-2] before:content-['_'] after:content-['_']">
                  {playerName}
                </span>
                            </Typography.Heading>
                        </div>
