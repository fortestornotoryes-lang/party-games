  ...
  <>
    ...
    <div className={`grid ${colClass} gap-1.5 px-[3px]`}>
      {Array.from({ length: cols }, (_, c) => {
        ...
        const showPopDot =
          isPopOut && action === C4Action.Pop && !gameOver && canPopColumn(board, c, current);
        return (
            ...
            <AnimatePresence>
              ...
              {!!showPopDot && (
                <motion.div
                  ...
                  exit={{ opacity: 0, scale: 0.4 }}
                  transition={{ duration: 0.13 }}
                  className="aspect-square w-[45%] rounded-full bg-white/80"
                />
              )}
            </AnimatePresence>
          </div>
    ...
    {/* Board */}
    <div
      className={`rounded-premium-md border-2 p-[10px] transition-colors duration-500 ${
        win
          ? win.player === 1
      ...
      <div className={`grid ${colClass} gap-1.5`}>
        {Array.from({ length: rows }, (_, r) =>
          Array.from({ length: cols }, (_, c) => {
            ...
            const popTarget =
              isPopOut && action === C4Action.Pop && canPopColumn(board, c, current);
              ...
              >
                ...
                <div
                  className={`absolute inset-[3px] rounded-full border transition-colors duration-200 ${
                    popTarget ? 'border-white/20 bg-black/55' : 'border-white/6 bg-black/80'
                  }`}
                />
                  ...
                  {cell !== 0 && (
                    <motion.div
                      ...
                      className={`absolute inset-[3px] rounded-full ${
                        cell === 1
                          ? `to-premium-red bg-gradient-to-br from-[#FF5570] ${isWinCell ? 'shadow-[0_0_18px_rgba(255,46,77,0.95)]' : 'shadow-[0_2px_8px_rgba(0,0,0,0.5)]'}`
                          : `to-premium-yellow bg-gradient-to-br from-[#FFE55C] ${isWinCell ? 'shadow-[0_0_18px_rgba(255,204,31,0.95)]' : 'shadow-[0_2px_8px_rgba(0,0,0,0.5)]'}`
                      }`}
                    />
                  )}
                </AnimatePresence>
