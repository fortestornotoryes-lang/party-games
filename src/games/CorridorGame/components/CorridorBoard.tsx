...
export const CorridorBoard: React.FC<Props> = ({
    ...
    const wallOrientation = actionMode === ActionMode.WallH ? 'H' : actionMode === ActionMode.WallV ? 'V' : null;
        ...
        >
            ...
            {Array.from({length: 9}, (_, r) =>
                Array.from({length: 9}, (_, c) => {
                    ...
                    const isValidMove = actionMode === ActionMode.Move && moves.some(m => m.row === r && m.col === c);
                    return (
                        <g key={`c${r}${c}`}>
                            <rect
                    ...
                    Array.from({length: 8}, (_, c) => {
                        ...
                        if (wallOrientation === 'H') {
                            return (
                                <rect
                                    ...
                                    onClick={() => { onWallClick(r, c, 'H'); }}
                                    onMouseEnter={() => { onHover({r, c, o: 'H'}); }}
                        ...
                        return (
                            <rect
                                ...
                                fill={isHov ? `${curColor}bb` : `${curColor}35`}
                                onClick={() => { onWallClick(r, c, 'V'); }}
                                onMouseEnter={() => { onHover({r, c, o: 'V'}); }}
                                onMouseLeave={() => { onHover(null); }}
                                style={{cursor: 'pointer'}}
                            />
                        );
