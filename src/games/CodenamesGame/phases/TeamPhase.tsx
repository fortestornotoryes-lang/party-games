...
export const TeamPhase: React.FC<TeamPhaseProps> = ({
    cards, turn, clueWord, clueCount, guessesLeft, lastActionMsg, onCardClick, onEndTurn,
}) => {
            ...
            <div className="grid grid-cols-5 gap-1.5 flex-1 items-center relative">
                ...
                {cards.map(card => (
                    <button
                        ...
                        `}
                    >
                        <span className={`text-[10px] sm:text-xs font-black leading-tight wrap-break-word uppercase ${!card.revealed ? 'text-stone-800' : ''}`}>
                            {card.word}
                        </span>
                    </button>
                ))}
