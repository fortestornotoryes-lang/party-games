...
export const useMemoRiskGameLogic = (playerNames: string[], onBack: () => void) => {
  ...
  const [phase, setPhase] = usePersistedState<MemoRiskPhase>(K, 'phase', MemoRiskPhase.Playing, {
    save: (v) => v,
    load: (raw) =>
      raw === MemoRiskPhase.GameOver ? MemoRiskPhase.GameOver : MemoRiskPhase.Playing,
  });
  ...
  const {
    ...
    next: nextPlayer,
    reset: resetPlayers,
  } = usePlayerCycle(playerNames, playerIdxState);
  ...
  const handleNextTurn = useCallback(() => {
    ...
    const tookTarget =
      ...
      !!round &&
      cards.board.some(
        (c) => c?.state === MemoCardState.Collected && round.targets.includes(c.shape)
      );
