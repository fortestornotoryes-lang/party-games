...
type Phase = 'setup' | 'pass_captain' | 'captain_clues' | 'pass_enemy' | 'enemy_intercept' | 'pass_team' | 'team_guess' | 'reveal' | 'game_over';
      ...
      <div className="flex-1 flex flex-col p-4 max-w-lg mx-auto w-full pt-10">
        <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="grid gap-3">
                <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-2xl">
                   <h3 className="text-red-400 font-bold uppercase text-xs mb-2 flex items-center gap-2"><Users className="w-4 h-4"/> Красная Команда</h3>
                ...
                <div className="p-4 bg-blue-500/10 border border-blue-500/30 rounded-2xl">
                   <h3 className="text-blue-400 font-bold uppercase text-xs mb-2 flex items-center gap-2"><Users className="w-4 h-4"/> Синяя Команда</h3>
          ...
          {phase === 'pass_captain' && (
            ...
            >
              <div className={`p-8 rounded-[2rem] ${activeTeam === 'red' ? 'bg-red-500/10 border-red-500/20' : 'bg-blue-500/10 border-blue-500/20'} border shadow-2xl`}>
                 <KeyRound className={`w-24 h-24 ${activeTeam === 'red' ? 'text-red-500 drop-shadow-[0_0_15px_rgba(239,68,68,0.5)]' : 'text-blue-500 drop-shadow-[0_0_15px_rgba(59,130,246,0.5)]'}`} />
              </div>
              <div>
                <p className="text-sm uppercase tracking-widest text-gray-500 mb-2 font-bold">Раунд {round} • Шифровальщик</p>
                ...
                <div className="mt-4">
                  <span className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest ${activeTeam === 'red' ? 'bg-red-500/20 text-red-400' : 'bg-blue-500/20 text-blue-400'}`}>
                    Команда {activeTeam === 'red' ? 'Красных' : 'Синих'}
                  </span>
                </div>
              </div>
              <div className="w-full pt-8">
                 <PrimaryButton onClick={() => setPhase('captain_clues')} variant={activeTeam === 'red' ? 'red' : 'blue'} className="w-full h-16 text-lg tracking-widest">ПОКАЗАТЬ КОД</PrimaryButton>
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-2">
                {currentTeamState.words.map((w, i) => (
                  <div key={i} className="bg-white/5 border border-white/10 p-2 rounded relative overflow-hidden flex flex-col items-center justify-center h-20">
                    <span className="absolute left-2 top-2 text-[10px] text-gray-500 font-black">{i + 1}</span>
              ...
              <form onSubmit={handleCluesSubmit} className="space-y-4 flex-1">
                <p className="text-xs font-bold text-gray-400 text-center uppercase mt-4">Напишите ассоциации к словам кода</p>
                {currentCode.map((num, i) => (
                  <div key={i} className="flex gap-4 items-center">
                    <div className={`w-8 h-8 flex items-center justify-center font-black rounded-lg ${activeTeam === 'red' ? 'bg-red-500/20 text-red-400' : 'bg-blue-500/20 text-blue-400'}`}>
                      {num}
                    </div>
                    <input
                      type="text"
                ...
                <PrimaryButton type="submit" variant={activeTeam} className="w-full mt-6" disabled={clues.some(c=>c.trim()==='')}>
                  ЗАШИФРОВАТЬ
                </PrimaryButton>
              </form>
            </motion.div>
          ...
          {phase === 'pass_enemy' && (
            <motion.div
              ...
              className="flex-1 flex flex-col items-center justify-center text-center space-y-8"
            >
              <div className={`p-8 rounded-[2rem] ${enemyTeamColor === 'red' ? 'bg-red-500/10 border-red-500/20' : 'bg-blue-500/10 border-blue-500/20'} border shadow-2xl`}>
                 <AlertOctagon className={`w-24 h-24 ${enemyTeamColor === 'red' ? 'text-red-500 drop-shadow-[0_0_15px_rgba(239,68,68,0.5)]' : 'text-blue-500 drop-shadow-[0_0_15px_rgba(59,130,246,0.5)]'}`} />
              </div>
              <div>
                <p className="text-sm uppercase tracking-widest text-gray-500 mb-2 font-bold">Передайте телефон для перехвата</p>
                <div className="mt-4">
                  <span className={`px-5 py-2 rounded-full text-sm font-black uppercase tracking-widest ${enemyTeamColor === 'red' ? 'bg-red-500/20 text-red-400' : 'bg-blue-500/20 text-blue-400'}`}>
                    Командe {enemyTeamColor === 'red' ? 'Красных' : 'Синих'}
                  </span>
                </div>
              </div>
              <div className="w-full pt-8">
                 <PrimaryButton onClick={() => setPhase('enemy_intercept')} variant={enemyTeamColor} className="w-full h-16 text-lg tracking-widest">ПЕРЕХВАТИТЬ</PrimaryButton>
              </div>
            </motion.div>
          )}
          ...
          {phase === 'enemy_intercept' && (
            ...
            >
              <div className="text-center font-black">
                <p className={`text-[10px] tracking-widest uppercase ${enemyTeamColor === 'red' ? 'text-red-500' : 'text-blue-500'}`}>ВРЕМЯ ПЕРЕХВАТА</p>
              </div>

              <div className="bg-white/5 border border-white/10 p-4 rounded-xl">
                 <p className="text-xs text-gray-500 font-bold uppercase mb-2">Текущие подсказки врага</p>
                 <ul className="space-y-1">
                   {clues.map((c, i) => <li key={i} className="text-white font-bold">{i+1}. {c}</li>)}
                 </ul>
              </div>

              {currentTeamState.history.length > 0 && (
                <div className="bg-white/5 border border-white/10 p-4 rounded-xl">
                  <p className="text-xs text-gray-500 font-bold uppercase mb-2">История раундов врага</p>
                  <div className="space-y-2">
                    {currentTeamState.history.map((h, i) => (
                      <div key={i} className="text-xs">
                        <span className="text-purple-400 font-bold">Код {h.code.join('-')}</span>: {h.clues.join(', ')}
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <form onSubmit={handleInterceptSubmit} className="mt-4 space-y-4 flex-1">
                <p className="text-[10px] font-bold text-center text-gray-400 uppercase">Введите перехваченный код</p>
                ...
                <PrimaryButton type="submit" variant={enemyTeamColor} className="w-full">ПОДТВЕРДИТЬ ПЕРЕХВАТ</PrimaryButton>
              </form>
            </motion.div>
          )}
          ...
          {phase === 'pass_team' && (
            ...
            >
              <div className={`p-8 rounded-[2rem] ${activeTeam === 'red' ? 'bg-red-500/10 border-red-500/20' : 'bg-blue-500/10 border-blue-500/20'} border shadow-2xl`}>
                 <Users className={`w-24 h-24 ${activeTeam === 'red' ? 'text-red-500 drop-shadow-[0_0_15px_rgba(239,68,68,0.5)]' : 'text-blue-500 drop-shadow-[0_0_15px_rgba(59,130,246,0.5)]'}`} />
              </div>
              <div>
                <p className="text-sm uppercase tracking-widest text-gray-500 mb-2 font-bold">Передайте вашей команде</p>
                <div className="mt-4">
                  <span className={`px-5 py-2 rounded-full text-sm font-black uppercase tracking-widest ${activeTeam === 'red' ? 'bg-red-500/20 text-red-400' : 'bg-blue-500/20 text-blue-400'}`}>
                    Командe {activeTeam === 'red' ? 'Красных' : 'Синих'}
                  </span>
                </div>
              </div>
              <div className="w-full pt-8">
                 <PrimaryButton onClick={() => setPhase('team_guess')} variant={activeTeam} className="w-full h-16 text-lg tracking-widest">РАЗГАДАТЬ КОД</PrimaryButton>
              </div>
            </motion.div>
          )}
          ...
          {phase === 'team_guess' && (
            ...
            >
              <div className="grid grid-cols-2 gap-2 mb-4">
                {currentTeamState.words.map((w, i) => (
                  <div key={i} className="bg-white/5 border border-white/10 p-2 rounded relative overflow-hidden flex flex-col items-center justify-center h-16">
                    <span className="absolute left-2 top-2 text-[10px] text-gray-500 font-black">{i + 1}</span>
                    <span className="text-sm font-bold uppercase">{w}</span>
                  </div>
                ))}
              ...
              <form onSubmit={handleTeamGuessSubmit} className="mt-4 space-y-4 flex-1">
                <p className="text-[10px] font-bold text-center text-gray-400 uppercase">Введите ваш код</p>
                <div className="grid grid-cols-3 gap-2">
                  {[0,1,2].map(i => (
                    <input
                      key={i} type="number" min="1" max={wordCount} required
          ...
          {phase === 'reveal' && (
            ...
            >
              ...
              <div className="bg-white/5 border border-white/10 p-8 rounded-[2rem] w-full shadow-2xl relative overflow-hidden">
                 <div className="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-transparent via-emerald-500 to-transparent"></div>
                 <p className="text-xs text-gray-500 uppercase font-black tracking-widest mb-4">Истинный код {activeTeam === 'red' ? 'Красных' : 'Синих'}</p>
                 <h3 className="text-6xl font-black text-emerald-400 tracking-[0.25em]">{currentCode.join('')}</h3>
              </div>

              <div className="space-y-3 mt-8 w-full">
                {round > 1 && (
                  <div className="bg-white/5 border border-white/10 rounded-2xl p-5 flex flex-col w-full relative overflow-hidden">
                     {interceptGuess.join('') === currentCode.join('') && <div className="absolute inset-0 bg-red-500/10"></div>}
                     <div className="flex justify-between items-center w-full z-10">
                       <span className="text-xs text-gray-400 uppercase font-bold text-left">Перехват врага<br/><span className="text-white text-2xl tracking-[0.2em]">{interceptGuess.join('')}</span></span>
                       {interceptGuess.join('') === currentCode.join('') ? (
                          <span className="text-red-400 font-black px-4 py-2 bg-red-400/20 rounded-xl border border-red-500/30">ПЕРЕХВАТ</span>
                       ) : (
                          <span className="text-gray-500 font-bold px-4 py-2 bg-white/5 rounded-xl">МИМО</span>
                       )}
                     </div>
                  </div>
                )}
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl p-5 flex flex-col w-full relative overflow-hidden">
                     {teamGuess.join('') === currentCode.join('') && <div className="absolute inset-0 bg-emerald-500/10"></div>}
                     <div className="flex justify-between items-center w-full z-10">
                       <span className="text-xs text-gray-400 uppercase font-bold text-left">Своя команда<br/><span className="text-white text-2xl tracking-[0.2em]">{teamGuess.join('')}</span></span>
                       {teamGuess.join('') === currentCode.join('') ? (
                          <span className="text-emerald-400 font-black px-4 py-2 bg-emerald-400/20 rounded-xl border border-emerald-500/30">УСПЕХ</span>
                       ) : (
                          <span className="text-red-400 font-black px-4 py-2 bg-red-400/20 rounded-xl border border-red-500/30">ОШИБКА</span>
                       )}
                     </div>
                </div>
              </div>
              ...
              <div className="w-full pt-8">
                 <PrimaryButton onClick={continueAfterReveal} variant="white" className="w-full h-16 text-lg tracking-widest">ДАЛЬШЕ</PrimaryButton>
              </div>
            </motion.div>
          )}
