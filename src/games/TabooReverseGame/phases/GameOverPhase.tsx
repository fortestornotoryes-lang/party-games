...
export const GameOverPhase: React.FC<GameOverPhaseProps> = ({
  ...
  return (
    <motion.div
      ...
      className="mx-auto flex w-full max-w-sm flex-col items-center gap-8 p-6"
    >
      {/* ── Team result header ─────────────────────────────────────────────── */}
      {teams ? (
        <div className="w-full space-y-2 pt-4 text-center">
          <Trophy className="text-premium-yellow mx-auto mb-4 h-16 w-16 drop-shadow-[0_0_20px_rgba(234,179,8,0.4)]" />
          <p className="text-micro font-black tracking-[0.5em] text-white/30 uppercase">
            {t(`${NS.TABOO}.gameEnded`)}
          </p>
          {teamWinner === -1 ? (
            <h2 className="text-4xl font-black tracking-tighter text-white/70 uppercase italic">
              {t(`${NS.COMMON}.draw`)}!
            </h2>
          ) : (
            <h2
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            {([0, 1] as const).map((ti) => {
              ...
              return (
                <div
                  key={ti}
                  className={`rounded-premium-md space-y-2 border-2 p-4 ${
                    isWinner
                      ? ti === 0
                    ...
                    {[...teams[ti]]
                      ...
                      .map((p) => (
                        <div key={p} className="flex items-center justify-between">
                          <span className="text-label truncate font-bold text-white/50">{p}</span>
                          <span className="text-label ml-2 font-black text-white/40 tabular-nums">
                            {scores[p] ?? 0}
                          </span>
                        </div>
                      ))}
        ...
        <div className="space-y-2 pt-4 text-center">
          ...
          {hasWinner ? (
            <h2 className="text-premium-yellow text-4xl font-black tracking-tighter uppercase italic">
