...
export function runBunkerSimulation(runs: number, countHiddenTraits = true): SimReport {
    ...
    for (let i = 0; i < runs; i++) {
        ...
        data.push({
            ...
            eventCount, playerCount, bunkerSize,
            outcome: computeOutcome(res), finalResources: res,
            scenarioDelta, scenarioNet, teamDelta, teamNet, eventDelta, eventNet,
        });
    }
    ...
    const outcomesPct = Object.fromEntries(
        (Object.keys(outcomes) as SurvivalOutcome[]).map(k => [k, Math.round(outcomes[k] / runs * 100)])
    ...
    const scenarioStats: ScenarioStat[] = Array.from(scMap.entries())
        .map(([title, list]) => {
            ...
            const avgScore = Math.round(
                (oc.full_victory * 3 + oc.partial * 2 + oc.pyrrhic * 1) / list.length * 10
            ) / 10;
            return {
                title,
