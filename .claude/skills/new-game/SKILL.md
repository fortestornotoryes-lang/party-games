...
3. **Theme color** — one of `GameTheme` in `src/types.ts`: `red | green | sky | yellow | orange | purple | blue | pink | cyan | lime | teal | indigo`
...
## Step 1 — Add GameKey entry (`src/types/games.ts`)
...
(Named export — the lazy import in GameRegistry maps it to default.)
...
- **Reuse, don't re-implement (see hooks_and_patterns.md):** `usePlayerCycle` for turn rotation, `useTimer` for countdowns, `shuffle`/`pickRandom` from `@/utils/random`, `feedbackService.vibrate(VIBRATE.*)`, `TimerBar`, `ProgressDots`, `PlayingHeader`, `PassPhoneCard`, `PlayerScoreList`, `LeaderboardList`, `StopGameButton`, `DistributionFlow`, `canvas-confetti` for win celebrations.
- **Settings:** read `difficulty` / `mode` / `timerSeconds` from `useGameSettings()` (`@/contexts/GameSettingsContext`). Per-difficulty/per-mode numbers live in `constants.ts` as `Record<Difficulty, ...>` configs — no inline ternaries on difficulty.

---

## Step 3 — Register in GameRegistry (`src/registry/GameRegistry.tsx`)

Add an **exported** lazy import next to the others (GamePlayRoute imports the component from here):

```ts
export const <GameName>Game = lazy(() =>
    import('../games/<GameName>Game/<GameName>Game').then((m) => ({default: m.<GameName>Game}))
);
```

(Check how existing lazy consts are exported in the file and match it.)

Add an entry to `GAMES_REGISTRY` (shape = `GameMetadata` in `src/types/games.ts` — there is NO `setupStatus` and NO `instructions` field):
...
## Step 4 — Add routing (`src/router/GamePlayRoute.tsx`)
...
1. Add `<GameName>Game` to the import from `'../registry/GameRegistry'`.
...
Modes from `registry.modes` get rendered automatically; the chosen id arrives via `useGameSettings().mode`. The classic/default mode id is `CLASSIC_MODE_ID` from `src/types/games.ts`.

---

## Step 6 — Add instructions (`src/constants/instructions.ts`)
...
1. `src/i18n/keys.ts` — add the namespace: `<GAME_ID_UPPER>: '<game_id>',` to `NS`.
2. `src/i18n/types.ts` — add `export interface <GameName>Translations { ... }` (one field per string, comment with the RU original), add the optional key to `Translations` interface AND to its index-signature union.
3. `src/i18n/ru.ts` / `src/i18n/en.ts` — add the `<game_id>: {...}` blocks with full translations.
